...
import { Sparkles, Activity, ShieldCheck, Zap, Globe, Lock, Unlock, AlertCircle } from 'lucide-react';
import { useLanguage } from '../../contexts/LanguageContext';
import { useUser } from '../../contexts/UserContext';
import { getDailyHealthTip, isAIServiceAvailable } from '../../lib/gemini';
    ...
    const fetchTip = async () => {
      ...
      try {
        ...
        if (available) {
          ...
          setTip('Servicio no disponible');
        }
      } catch (error) {
        console.error('Error al obtener el consejo de salud:', error);
    ...
    <div className="w-full max-w-7xl mx-auto px-4 md:px-6 mb-12">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <motion.div 
          ...
          className={`${aiAvailable ? 'bg-primary text-on-primary' : 'bg-surface-container border border-outline-variant/50'} rounded-[32px] p-6 shadow-xl flex flex-col justify-between relative overflow-hidden group`}
        >
          <div className={`absolute top-0 right-0 p-6 ${aiAvailable ? 'opacity-5' : 'opacity-10'} group-hover:scale-110 transition-transform pointer-events-none`}>
            {aiAvailable ? <Zap className="w-24 h-24" /> : <AlertCircle className="w-24 h-24 text-error" />}
          </div>
          <div className="relative z-10 flex items-center justify-between mb-4">
            <div className={`p-3 rounded-2xl border ${aiAvailable ? 'bg-on-primary/10 border-on-primary/20' : 'bg-error/10 border-error/20'}`}>
              {aiAvailable ? <Zap className={`w-6 h-6 ${aiAvailable ? 'text-on-primary' : 'text-error'}`} /> : <AlertCircle className="w-6 h-6 text-error" />}
            </div>
            <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${
              ...
              {aiAvailable ? (isPremium ? 'PREMIUM' : t('ai.status.active')) : 'NO CONFIGURADO'}
            </span>
          </div>
          <div className="relative z-10">
            <h3 className={`text-xl font-display font-black mb-1 ${aiAvailable ? 'text-on-primary' : 'text-on-surface'}`}>
              {t('ai.status.title')}
            </h3>
            <div className="flex flex-col gap-1">
              <span className={`text-[10px] font-mono font-bold uppercase tracking-wider ${aiAvailable ? 'text-on-primary/70' : 'text-on-surface-variant/60'}`}>
                {aiAvailable 
              ...
              <span className={`text-[10px] font-mono font-bold uppercase tracking-wider ${aiAvailable ? 'text-on-primary/70' : 'text-on-surface-variant/60'}`}>
                {aiAvailable ? t('ai.status.latency') : 'Falta configurar API Key'}
              </span>
            </div>
          </div>
        </motion.div>
            ...
            {isLoading ? (
              ...
              <div className="flex items-center gap-3 text-sm">
                ...
                <p className="opacity-80">
                  El servicio de IA no está configurado. Contáctanos para más información.
                </p>
              </div>
            )}
